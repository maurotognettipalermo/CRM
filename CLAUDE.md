...
- **Tablas sin migración ALTER**: las que no existían en versiones anteriores se crean solo por `schema.sql`. Las migraciones ALTER en `database.js` solo afectan columnas añadidas a tablas ya existentes (apartamentos, reservas, portales, horas_extra, propiedades_venta, usuarios, facturas, contratos, contrato_cuotas).
