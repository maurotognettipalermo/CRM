...
| POST   | /api/ajustes/razones-sociales/:id/logo  | Subir logo (multipart, campo `logo`); .jpg/.jpeg/.png/.webp/.svg; borra el anterior; devuelve { ok, logo_url } |
...
- **razones_sociales**: datos de facturación, una fila por razón social (cada una es una
  ...
  banco, IBAN…) + `logo_url` (vía `migrarRazones`; logo subido a `public/uploads/razones-sociales/`).
  `routes/ajustes.js` define `RS_CAMPOS` como punto de verdad para INSERT/UPDATE (logo_url va aparte,
  por el endpoint de subida).
